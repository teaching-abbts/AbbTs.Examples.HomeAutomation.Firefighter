...
{
    public static void Initialize(RuntimePaths paths, IReadOnlyCollection<SmartHomeInstance> smartHomes)
    {
        Directory.CreateDirectory(paths.StateDir);

        if (!File.Exists(paths.SmartHomeJarPath))
        {
            throw new FileNotFoundException($"SmartHome jar was not found at '{paths.SmartHomeJarPath}'.");
        }
