...
public sealed class BuildContext : FrostingContext
{
    ...
    : base(context)
  {
    ...
    WebhostProjectPath = Path.Combine(
      RepoRoot,
  ...
  public RuntimePaths GetRuntimePaths(RuntimeMode mode)
  {
    return mode switch
    {
      ...
      RuntimeMode.Artifacts => new RuntimePaths
      {
        ...
        WebhostPublishedDllPath = Path.Combine(
          ArtifactsRoot,
          "webhost",
          "AbbTs.Examples.HomeAutomation.Firefighter.Webhost.dll"
        ),
      },
  ...
  public SmartHomeRuntimeSettings LoadSmartHomeSettings(string smartHomesConfigFile)
  {
    if (!File.Exists(smartHomesConfigFile))
    {
      throw new FileNotFoundException(
        $"Smart-home settings file was not found at '{smartHomesConfigFile}'."
      );
    }

    var json = File.ReadAllText(smartHomesConfigFile);
    var settings =
      JsonSerializer.Deserialize<SmartHomeRuntimeSettings>(json, JsonOptions)
  ...
  )
  {
    Directory.CreateDirectory(
      Path.GetDirectoryName(processesFile)
        ?? throw new InvalidOperationException("Invalid processes file path.")
    );
