...
public sealed class SmartQuartierDashboardHistoryBroadcasterActor : ReceiveActor, IWithTimers
{
        ...
        ILogger<SmartQuartierDashboardHistoryBroadcasterActor> logger)
    {
        ...
        ReceiveAsync<BroadcastDashboardHistory>(async _ =>
        {
            ...
            try
            {
                ...
                var history = await analyticsActor.Ask<SmartQuartierHistoryResponse>(new GetSmartQuartierHistory(), AskTimeout);
    ...
    {
        return await _actorSystem.ActorSelection(SmartQuartierAnalyticsActor.ActorPath).ResolveOne(AskTimeout);
    }
