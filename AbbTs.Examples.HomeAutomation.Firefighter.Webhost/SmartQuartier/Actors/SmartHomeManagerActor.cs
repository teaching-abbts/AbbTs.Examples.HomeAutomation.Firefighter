...
public sealed class SmartHomeManagerActor : ReceiveActor
{
    ...
    private readonly Dictionary<string, IActorRef> _smartHomes = new(StringComparer.OrdinalIgnoreCase);
    ...
    public SmartHomeManagerActor()
    {
        ...
        ReceiveAsync<SendSmartHomeDashboardCommand>(async message =>
        {
            ...
            if (!_smartHomes.TryGetValue(message.SmartHomeId, out var actor))
            {
                replyTo.Tell(new SmartHomeCommandResult(false, $"Unknown smart-home '{message.SmartHomeId}'."));
                return;
            }
        ...
        {
            var replyTo = Sender;
            var tasks = _smartHomes.Values
                .Select(actor => actor.Ask<SmartHomeSummary>(new GetSmartHomeSummary(), AskTimeout));

            var summaries = await Task.WhenAll(tasks);
            replyTo.Tell(summaries.OrderBy(summary => summary.Id, StringComparer.OrdinalIgnoreCase).ToArray());
        });
    ...
    private IActorRef GetOrCreateActor(SmartHomeRegistration registration)
    {
        ...
        var actor = Context.ActorOf(
            Props.Create(() => new SmartHomeActor(registration)),
            actorName);
