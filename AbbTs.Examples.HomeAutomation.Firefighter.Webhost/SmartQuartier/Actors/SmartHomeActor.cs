...
public sealed class SmartHomeActor : ReceiveActor, ICancelable
{
    ...
    private void RecordEnvelope(SmartHomeGatewayEnvelope envelope)
    {
        ...
        if (string.Equals(envelope.MessageType, "send registration", StringComparison.OrdinalIgnoreCase)
    ...
    private static async Task ClosePreviousSocketAsync(WebSocket socket)
    {
        try
        {
            ...
            {
                await socket.CloseAsync(WebSocketCloseStatus.NormalClosure, "Replaced by newer session", CancellationToken.None);
            }
        }
