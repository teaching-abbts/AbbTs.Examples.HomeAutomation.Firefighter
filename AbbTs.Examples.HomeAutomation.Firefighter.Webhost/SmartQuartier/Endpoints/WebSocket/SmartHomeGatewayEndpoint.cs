...
public static class SmartHomeGatewayEndpoint
{
    public static void MapSmartHomeGatewayEndpoints(this WebApplication app)
    {
        ...
        app.Map("/smart-home/ws", async (HttpContext context, ISmartHomeGateway gateway, CancellationToken cancellationToken) =>
        {
            if (!context.WebSockets.IsWebSocketRequest)
            {
                context.Response.StatusCode = StatusCodes.Status400BadRequest;
            ...
            using var webSocket = await context.WebSockets.AcceptWebSocketAsync();
            await gateway.HandleDashboardSessionAsync(webSocket, cancellationToken);
        });
    }
}
