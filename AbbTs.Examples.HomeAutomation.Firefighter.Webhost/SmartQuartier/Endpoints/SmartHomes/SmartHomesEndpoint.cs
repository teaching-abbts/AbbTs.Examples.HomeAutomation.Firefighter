...
public static class SmartHomesEndpoint
{
    public static RouteGroupBuilder MapSmartHomesEndpoints(this WebApplication app)
    {
        ...
        group.MapGet("/", async (ISmartHomeGateway gateway, CancellationToken cancellationToken) =>
            {
        ...
        group.MapGet("/{smartHomeId}", async (string smartHomeId, ISmartHomeGateway gateway, CancellationToken cancellationToken) =>
            {
